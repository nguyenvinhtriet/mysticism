...
function IndicatorBox({ label, value, description, highlight = false, onHover }: { label: string, value: number, description?: string, highlight?: boolean, onHover?: () => void }) {
  return (
    <div 
      ...
      className={`flex flex-col items-center justify-center p-2 md:p-3 rounded-xl border transition-all cursor-pointer hover:scale-105 ${highlight ? 'bg-violet-600 text-white border-violet-700 shadow-md hover:bg-violet-500' : 'bg-slate-900 text-slate-100 border-slate-800 shadow-sm hover:border-violet-300 hover:shadow-md'}`}
    >
      <span className={`text-[10px] md:text-xs text-center mb-1 font-medium ${highlight ? 'text-violet-200' : 'text-slate-500'}`}>{label}</span>
      <span className={`text-xl md:text-2xl font-black ${highlight ? 'text-white' : 'text-violet-600'}`}>{value}</span>
      {description && <span className={`text-[8px] md:text-[10px] text-center mt-1 leading-tight ${highlight ? 'text-violet-200' : 'text-slate-500'}`}>{description}</span>}
    </div>
  );
}
...
function ReadingCard({ title, value, description, index, isActive }: { title: string, value: number, description: string, index: number, isActive: boolean }) {
  return (
    <div className={`bg-slate-900 rounded-2xl p-5 border transition-all flex flex-col h-full ${isActive ? 'border-violet-400 shadow-lg ring-2 ring-violet-900/50 scale-[1.02]' : 'border-fuchsia-900/40 shadow-sm hover:shadow-md'}`}>
      <div className="flex items-center gap-4 mb-3">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-xl shrink-0 border transition-colors ${isActive ? 'bg-violet-900/50 text-violet-700 border-violet-200' : 'bg-fuchsia-900/40 text-fuchsia-300 border-fuchsia-800/50'}`}>
          {value}
        </div>
        <h3 className={`text-lg font-bold m-0 leading-tight ${isActive ? 'text-violet-100' : 'text-fuchsia-100'}`}>{index}. {title}</h3>
      </div>
      <p className={`text-sm leading-relaxed m-0 flex-grow ${isActive ? 'text-slate-200' : 'text-fuchsia-200'}`}>
        {description}
      </p>
    </div>
  );
...
function NumerologyContent() {
  ...
  const previewContent = (
    <div className="bg-fuchsia-950/30 rounded-3xl shadow-xl p-6 md:p-8 border border-fuchsia-800/50">
      <h2 className="text-2xl font-bold text-fuchsia-100 mb-4 border-b border-fuchsia-800/50 pb-4">Tổng quan Thần Số Học</h2>
      <div className="prose prose-amber max-w-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div className="bg-slate-900/60 p-4 rounded-xl border border-fuchsia-900/40">
            <h3 className="text-lg font-bold text-fuchsia-100 mt-0 mb-1">Số Chủ Đạo {lifePath}</h3>
            <p className="text-fuchsia-200 text-sm mb-0">Đại diện cho con đường bạn sẽ đi, những bài học và cơ hội chính trong cuộc đời.</p>
          </div>
          <div className="bg-slate-900/60 p-4 rounded-xl border border-fuchsia-900/40">
            <h3 className="text-lg font-bold text-fuchsia-100 mt-0 mb-1">Sứ Mệnh {destiny}</h3>
            <p className="text-fuchsia-200 text-sm mb-0">Mục đích bạn đến với cuộc đời này và những gì bạn cần hoàn thành.</p>
          </div>
          <div className="bg-slate-900/60 p-4 rounded-xl border border-fuchsia-900/40">
            <h3 className="text-lg font-bold text-fuchsia-100 mt-0 mb-1">Nội Tâm {soulUrge}</h3>
            <p className="text-fuchsia-200 text-sm mb-0">Khát khao sâu kín nhất bên trong linh hồn mang lại cho bạn hạnh phúc thực sự.</p>
          </div>
          <div className="bg-slate-900/60 p-4 rounded-xl border border-fuchsia-900/40">
            <h3 className="text-lg font-bold text-fuchsia-100 mt-0 mb-1">Tương Tác {personality}</h3>
            <p className="text-fuchsia-200 text-sm mb-0">Cách người khác nhìn nhận bạn trong lần đầu gặp gỡ, ấn tượng bên ngoài của bạn.</p>
          </div>
          <div className="bg-slate-900/60 p-4 rounded-xl border border-fuchsia-900/40">
            <h3 className="text-lg font-bold text-fuchsia-100 mt-0 mb-1">Thái Độ {attitude}</h3>
            <p className="text-fuchsia-200 text-sm mb-0">Cách bạn phản ứng với thế giới xung quanh, đặc biệt là trong những tình huống mới.</p>
          </div>
          <div className="bg-slate-900/60 p-4 rounded-xl border border-fuchsia-900/40">
            <h3 className="text-lg font-bold text-fuchsia-100 mt-0 mb-1">Trưởng Thành {maturity}</h3>
            <p className="text-fuchsia-200 text-sm mb-0">Năng lượng sẽ ảnh hưởng mạnh mẽ đến bạn từ độ tuổi 35-40 trở đi.</p>
          </div>
        </div>
        <p className="text-fuchsia-200 mt-6 italic text-sm text-center">
  ...
  const unlockedPreviewContent = (
    <div className="bg-fuchsia-950/30 rounded-3xl shadow-xl p-6 md:p-8 border border-fuchsia-800/50">
      ...
      <div className="prose prose-amber max-w-none">
        ...
        <div className="mt-6 p-4 bg-slate-900/80 rounded-xl border border-fuchsia-800/50 text-fuchsia-100">
  ...
  return (
    <main className="min-h-screen bg-slate-950 pb-20">
      <div className="bg-slate-950 text-white pt-12 relative overflow-hidden pb-24 px-4 rounded-b-[3rem] shadow-lg">
        ...
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-b from-violet-900/50 to-slate-950 pointer-events-none"></div>
        <div className="max-w-7xl mx-auto relative z-10">
          <button 
            onClick={() => router.push('/')}
            className="flex items-center gap-2 text-violet-200 hover:text-white transition-colors mb-6"
          >
            <ArrowLeft size={20} />
            Quay lại
          </button>
          ...
          <p className="text-violet-200 text-lg">
            Khám phá ý nghĩa các con số của <span className="font-semibold text-white uppercase">{name}</span>, sinh ngày <span className="font-semibold text-white">{new Date(dob).toLocaleDateString('vi-VN')}</span>
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-16">
        <div className="flex flex-row gap-6">
          ...
          <div className="w-1/3 lg:w-1/4 shrink-0">
            <div className="bg-slate-900 rounded-3xl shadow-xl p-4 border border-slate-800 sticky top-6">
              <div className="text-center mb-4">
                <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Họ và tên</div>
                <div className="font-bold text-violet-100 uppercase text-sm break-words bg-violet-950/50 py-2 px-3 rounded-lg">
                  {name}
                </div>
              </div>
          ...
          <div className="w-2/3 lg:w-3/4">
            <UnlockPremium name={name} dob={dob} type="numerology" preview={previewContent} unlockedPreview={unlockedPreviewContent}>
              <div className="bg-fuchsia-950/30 rounded-3xl shadow-xl p-6 md:p-8 border border-fuchsia-800/50">
                <h2 className="text-2xl font-bold text-fuchsia-100 mb-6 border-b border-fuchsia-800/50 pb-4">Luận giải chuyên sâu</h2>
                
                <div className="bg-slate-900 p-6 md:p-8 rounded-3xl border border-violet-900/50 shadow-sm mb-8 min-h-[300px] flex flex-col justify-center transition-all">
                  {activeCard === null ? (
                    <div className="text-center text-slate-500 flex flex-col items-center justify-center h-full py-12">
                      <div className="w-16 h-16 bg-violet-950/50 text-violet-300 rounded-full flex items-center justify-center mb-4">
                      ...
                      <p className="text-lg font-medium text-slate-300">Di chuột vào các chỉ số bên trái để xem luận giải chi tiết</p>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                      {activeCard === 1 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Số Chủ Đạo (Đường Đời)</h3>
                              <p className="text-violet-600 font-medium">Con số quan trọng nhất của bạn</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Số Chủ Đạo đại diện cho <strong className="text-violet-700">con đường bạn sẽ đi</strong> trong cuộc đời này, những bài học bạn cần học và những cơ hội bạn sẽ gặp phải.</p>
                      ...
                      {activeCard === 2 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Sứ Mệnh</h3>
                              <p className="text-violet-600 font-medium">Mục đích bạn đến với cuộc đời này</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Chỉ số Sứ Mệnh cho biết <strong className="text-violet-700">mục đích sống</strong> của bạn, điều mà bạn được sinh ra để thực hiện và đóng góp cho thế giới.</p>
                            <p>Mang năng lượng số <strong>{destiny}</strong>, sứ mệnh của bạn gắn liền với việc <span className="bg-violet-900/50 text-violet-200 px-1 rounded">truyền cảm hứng và dẫn dắt</span> người khác. Bạn có khả năng nhìn thấy bức tranh toàn cảnh và đưa ra những giải pháp sáng tạo.</p>
                      ...
                      {activeCard === 3 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Nội Tâm (Linh Hồn)</h3>
                              <p className="text-violet-600 font-medium">Khát khao sâu kín nhất</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Nội Tâm tiết lộ những <strong className="text-violet-700">khát khao sâu kín nhất</strong> bên trong bạn, những điều thực sự mang lại cho bạn hạnh phúc và sự thỏa mãn từ tận đáy lòng.</p>
                      ...
                      {activeCard === 4 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Tương Tác (Nhân Cách)</h3>
                              <p className="text-violet-600 font-medium">Cách thế giới nhìn nhận bạn</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Chỉ số Tương Tác là <strong className="text-violet-700">cách người khác nhìn nhận bạn</strong> trong lần đầu gặp gỡ, ấn tượng bên ngoài mà bạn tạo ra.</p>
                            
                            <div className="mt-6 space-y-6">
                              <div>
                                <h4 className="text-lg font-bold text-violet-200 flex items-center gap-2">
                                  <span className="w-6 h-6 rounded-full bg-violet-900/50 text-violet-600 flex items-center justify-center text-sm">1</span>
                                  Khí chất bề ngoài (The Aura)
                                </h4>
                                <p className="mt-2 text-slate-300">{getPersonalityInterpretation(personality).aura}</p>
                              </div>
                              
                              <div>
                                <h4 className="text-lg font-bold text-violet-200 flex items-center gap-2">
                                  <span className="w-6 h-6 rounded-full bg-violet-900/50 text-violet-600 flex items-center justify-center text-sm">2</span>
                                  Nam châm thu hút (Phong cách tương tác)
                                </h4>
                                <p className="mt-2 text-slate-300">{getPersonalityInterpretation(personality).magnet}</p>
                              </div>

                              <div>
                                <h4 className="text-lg font-bold text-violet-200 flex items-center gap-2">
                                  <span className="w-6 h-6 rounded-full bg-violet-900/50 text-violet-600 flex items-center justify-center text-sm">3</span>
                                  Góc khuất và Sự hiểu lầm (The Illusion)
                                </h4>
                                <p className="mt-2 text-slate-300">{getPersonalityInterpretation(personality).illusion}</p>
                              </div>

                              <div className="bg-violet-950/50 p-4 rounded-xl border border-violet-900/50">
                                <h4 className="text-lg font-bold text-violet-100 flex items-center gap-2">
                                  <span className="w-6 h-6 rounded-full bg-violet-200 text-violet-700 flex items-center justify-center text-sm">4</span>
                                  Lời khuyên Chuyển hóa
                                </h4>
                                <p className="mt-2 text-violet-200 font-medium">{getPersonalityInterpretation(personality).advice}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {activeCard === 5 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Thái Độ</h3>
                              <p className="text-violet-600 font-medium">Phản ứng với thế giới</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Chỉ số Thái Độ thể hiện <strong className="text-violet-700">cách bạn phản ứng</strong> với những tình huống bất ngờ hoặc những thử thách đầu tiên trong cuộc sống.</p>
                            <p>Với số <strong>{attitude}</strong>, bạn thường đối mặt với khó khăn bằng sự <span className="bg-violet-900/50 text-violet-200 px-1 rounded">lạc quan, kiên cường và thực tế</span>. Bạn không dễ bị đánh gục bởi nghịch cảnh.</p>
                      ...
                      {activeCard === 6 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Trưởng Thành</h3>
                              <p className="text-violet-600 font-medium">Tiềm năng nửa sau cuộc đời</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Chỉ số Trưởng Thành cho biết <strong className="text-violet-700">tiềm năng và định hướng</strong> của bạn trong nửa sau của cuộc đời (thường từ 35-40 tuổi trở đi).</p>
                      ...
                      {activeCard === 7 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Cân Bằng</h3>
                              <p className="text-violet-600 font-medium">Chìa khóa vượt qua khủng hoảng</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Chỉ số Cân Bằng hướng dẫn bạn <strong className="text-violet-700">cách lấy lại sự ổn định</strong> khi đối mặt với căng thẳng và thử thách tột độ.</p>
                      ...
                      {activeCard === 8 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Nội Cảm (Đam Mê Tiềm Ẩn)</h3>
                              <p className="text-violet-600 font-medium">Tài năng ẩn giấu</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Nội Cảm tiết lộ những <strong className="text-violet-700">tài năng và đam mê tiềm ẩn</strong> mà bạn có thể chưa nhận ra hoặc chưa phát huy hết.</p>
                      ...
                      {activeCard === 9 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Chỉ Số Trí Tuệ</h3>
                              <p className="text-violet-600 font-medium">Cách bạn tư duy</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Chỉ Số Trí Tuệ cho biết <strong className="text-violet-700">cách bạn suy nghĩ</strong>, phân tích và giải quyết vấn đề trong cuộc sống hàng ngày.</p>
                      ...
                      {activeCard === 10 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Năng Lực Tiếp Cận</h3>
                              <p className="text-violet-600 font-medium">Bước đi đầu tiên</p>
                            </div>
                          </div>
                          <div className="prose prose-orange max-w-none text-slate-200 leading-relaxed">
                            <p>Năng Lực Tiếp Cận cho thấy <strong className="text-violet-700">cách bạn bắt đầu</strong> một dự án mới hoặc tiếp cận một cơ hội mới.</p>
                      ...
                      {activeCard === 11 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Năm Cá Nhân</h3>
                      ...
                      {activeCard === 12 && (
                        <div>
                          <div className="flex items-center gap-4 mb-6 border-b border-violet-900/50 pb-4">
                            ...
                            <div>
                              <h3 className="text-2xl font-bold text-violet-100 m-0">Tháng Cá Nhân</h3>
                ...
                <div className="bg-slate-900 p-6 md:p-8 rounded-3xl border border-fuchsia-900/40 shadow-sm">
                  <h3 className="text-xl font-bold text-fuchsia-100 mb-2">Biểu đồ Đỉnh Cao Cuộc Đời (Kim Tự Tháp)</h3>
                    ...
                    {pinnacles && (
                      <>
                        <div className="w-1/5 flex flex-col items-center relative z-10 px-1">
                          <div className="text-xs font-bold mb-3 bg-fuchsia-950/30 px-3 py-1.5 rounded-lg border border-fuchsia-800/50 shadow-sm text-fuchsia-100">{pinnacles.years[0]}</div>
                          ...
                          <div className="text-[10px] font-bold mt-2 text-slate-300">0 &gt;&gt; {pinnacles.ages[0]}</div>
                          <div className="h-16 w-full bg-fuchsia-900/40/60 rounded-t-xl mt-2 border-t border-x border-fuchsia-800/50/50"></div>
                        </div>
                        <div className="w-1/5 flex flex-col items-center relative z-20 px-1">
                          <div className="text-xs font-bold mb-3 bg-fuchsia-950/30 px-3 py-1.5 rounded-lg border border-fuchsia-800/50 shadow-sm text-fuchsia-100">{pinnacles.years[1]}</div>
                          ...
                          <div className="text-[10px] font-bold mt-2 text-slate-300">{pinnacles.ages[0] + 1} &gt;&gt; {pinnacles.ages[1]}</div>
                        ...
                        <div className="w-1/5 flex flex-col items-center relative z-30 px-1">
                          <div className="text-xs font-bold mb-3 bg-fuchsia-950/30 px-3 py-1.5 rounded-lg border border-fuchsia-800/50 shadow-sm text-fuchsia-100">{pinnacles.years[2]}</div>
                          ...
                          <div className="text-[10px] font-bold mt-2 text-slate-300">{pinnacles.ages[1] + 1} &gt;&gt; {pinnacles.ages[2]}</div>
                          <div className="h-32 w-full bg-fuchsia-700/50/60 rounded-t-xl mt-2 border-t border-x border-fuchsia-600/50/50"></div>
                        </div>
                        <div className="w-1/5 flex flex-col items-center relative z-20 px-1">
                          <div className="text-xs font-bold mb-3 bg-fuchsia-950/30 px-3 py-1.5 rounded-lg border border-fuchsia-800/50 shadow-sm text-fuchsia-100">{pinnacles.years[3]}</div>
                          ...
                          <div className="text-[10px] font-bold mt-2 text-slate-300">{pinnacles.ages[2] + 1} &gt;&gt; {pinnacles.ages[3]}</div>
                          <div className="h-24 w-full bg-fuchsia-800/50/60 rounded-t-xl mt-2 border-t border-x border-fuchsia-700/50/50"></div>
                        </div>
                        <div className="w-1/5 flex flex-col items-center relative z-10 px-1">
                          <div className="text-xs font-bold mb-3 bg-fuchsia-950/30 px-3 py-1.5 rounded-lg border border-fuchsia-800/50 shadow-sm text-fuchsia-100">{pinnacles.years[3] + 1}+</div>
                          ...
                          <div className="text-[10px] font-bold mt-2 text-slate-300">{pinnacles.ages[3] + 1}+</div>
                          <div className="h-16 w-full bg-fuchsia-900/40/60 rounded-t-xl mt-2 border-t border-x border-fuchsia-800/50/50"></div>
                        </div>
                      </>
                    )}
                  ...
                  <div className="mt-8 space-y-6">
                    <div className="bg-fuchsia-950/30 p-5 rounded-xl border border-fuchsia-800/50">
                      <h4 className="font-bold text-fuchsia-100 mb-4">Ý nghĩa các Đỉnh Cao</h4>
                      <div className="space-y-6 text-sm text-fuchsia-200">
                        <div className="border-b border-fuchsia-800/50/50 pb-4">
                  ...
                  <div className="mt-12 space-y-8">
                    {/* Personal Year Table */}
                    <div className="border border-slate-700 rounded-lg overflow-hidden bg-slate-900 shadow-sm">
                      <div className="bg-slate-800 text-center font-bold py-3 border-b border-slate-700 text-slate-100 flex items-center justify-center gap-2">
                        NĂM CÁ NHÂN
                        <button 
                          onClick={() => setShowNumberMeanings(!showNumberMeanings)} 
                          className="flex items-center gap-1.5 text-xs font-medium bg-violet-900/40 text-violet-200 hover:bg-violet-800/60 hover:text-white px-2.5 py-1 rounded-full transition-all border border-violet-700/50 ml-2"
                      ...
                      {showNumberMeanings && (
                        <div className="p-4 bg-violet-950/50 border-b border-slate-800 text-sm text-slate-200 animate-in fade-in slide-in-from-top-2">
                          <h5 className="font-bold text-violet-100 mb-2">Ý nghĩa các con số (1-9)</h5>
                      ...
                      <div className="p-4">
                        <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-9 gap-2">
                          {Array.from({ length: 9 }).map((_, i) => {
                            ...
                            return (
                              <div key={year} className={`rounded-lg overflow-hidden flex flex-col ${isCurrent ? 'ring-2 ring-violet-500 shadow-md transform scale-105 z-10' : 'border border-slate-800'}`}>
                                <div className={`text-center py-2 font-bold ${isPeak && !isCurrent ? 'bg-red-500 text-white' : isCurrent ? 'bg-violet-600 text-white' : 'bg-slate-800 text-slate-200'}`}>
                                  {pYear}
                                </div>
                                <div className={`text-center py-2 text-sm ${isCurrent ? 'bg-violet-950/50 font-bold text-violet-100' : 'bg-slate-900 text-slate-300'}`}>
                                  {year}
                                </div>
                              </div>
                            );
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="border border-slate-700 rounded-lg overflow-hidden bg-slate-900 shadow-sm flex flex-col">
                        <div className="bg-slate-800 text-center font-bold py-3 border-b border-slate-700 text-slate-100 uppercase flex items-center justify-center gap-2 flex-wrap">
                        ...
                        <div className="p-4 bg-slate-950 border-b border-slate-800 flex-grow">
                          <p className="text-sm text-slate-200">
                        ...
                        <div className="grid grid-cols-6 gap-1 p-2 bg-slate-900">
                          {Array.from({ length: 12 }).map((_, i) => {
                            ...
                            return (
                              <div key={month} className={`border rounded-md flex flex-col overflow-hidden ${isCurrentMonth ? 'ring-2 ring-violet-500 border-transparent' : 'border-slate-800'}`}>
                                <div className={`text-center py-1 font-bold text-sm ${isPeak && !isCurrentMonth ? 'bg-red-500 text-white' : isCurrentMonth ? 'bg-violet-600 text-white' : 'bg-slate-800 text-slate-200'}`}>
                                  {pMonth}
                                </div>
                                <div className={`text-center py-1 text-xs ${isCurrentMonth ? 'bg-violet-950/50 font-bold text-violet-100' : 'bg-slate-900 text-slate-300'}`}>
                                  T{month}
                                </div>
                              </div>
                            );
                      ...
                      <div className="border border-slate-700 rounded-lg overflow-hidden bg-slate-900 shadow-sm flex flex-col">
                        ...
                        <div className="grid grid-cols-6 gap-1 p-2 bg-slate-900">
                          {Array.from({ length: 12 }).map((_, i) => {
                            ...
                            return (
                              <div key={month} className="border border-slate-800 rounded-md flex flex-col overflow-hidden">
                                <div className={`text-center py-1 font-bold text-sm ${isPeak ? 'bg-red-500 text-white' : 'bg-slate-800 text-slate-200'}`}>
                                  {pMonth}
                                </div>
                                <div className="text-center py-1 text-xs bg-slate-900 text-slate-300">
                                  T{month}
                                </div>
                              </div>
                            );
                    ...
                    <div className="space-y-6">
                      <div className="bg-violet-950/50 p-6 rounded-xl border border-violet-900/50 shadow-sm">
                        <h4 className="font-bold text-xl text-violet-100 mb-4">Chi tiết Năm Cá Nhân {new Date().getFullYear()} (Số {calculatePersonalYear(dob, new Date().getFullYear())})</h4>
                        <div className="prose prose-orange max-w-none text-violet-200 text-sm">
                          <p>{getPersonalYearMeaning(calculatePersonalYear(dob, new Date().getFullYear()))}</p>
                        </div>
                        
                        <h5 className="font-bold text-violet-200 mt-6 mb-3">Tháng Cá Nhân Hiện Tại (Tháng {new Date().getMonth() + 1} - Số {calculatePersonalMonth(calculatePersonalYear(dob, new Date().getFullYear()), new Date().getMonth() + 1)})</h5>
                        <div className="bg-slate-900 p-4 rounded-lg border border-violet-900/50 text-sm text-slate-200">
                      ...
                      <div className="bg-slate-950 p-6 rounded-xl border border-slate-800 shadow-sm">
                        <h4 className="font-bold text-xl text-slate-100 mb-4">Chi tiết Năm Cá Nhân {new Date().getFullYear() + 1} (Số {calculatePersonalYear(dob, new Date().getFullYear() + 1)})</h4>
                        <div className="prose prose-slate max-w-none text-slate-200 text-sm">
                          <p>{getPersonalYearMeaning(calculatePersonalYear(dob, new Date().getFullYear() + 1))}</p>
                        </div>
                        
                        <h5 className="font-bold text-slate-200 mt-6 mb-3">Tháng Cá Nhân Đầu Tiên (Tháng 1 - Số {calculatePersonalMonth(calculatePersonalYear(dob, new Date().getFullYear() + 1), 1)})</h5>
